...
def main():
    ...
    imgTestingNumbers = cv2.imread("Page1.jpg")          # read in testing numbers image
    ...
    for contourWithData in validContoursWithData:            # for each contour
        ...
        print(contourWithData.intRectWidth >= 12 and (contourWithData.intRectHeight >= 22))
        if contourWithData.intRectWidth >= 12 and (contourWithData.intRectHeight >= 22):
...
def readcsv(rownum):
    ...
    imgROIResized= num_data[rownum:rownum +1,1:]
...
def DassModel():
    ...
    #validContoursWithData.sort(key = operator.attrgetter("intRectX"))         # sort contours from left to right
    validContoursWithData.sort(key = lambda x: (x.intRectY, x.intRectX))         # sort contours from left to right
    #validContoursWithData.sort(key = operator.attrgetter("intRectY"))         # sort contours from left to right
    ...
    for contourWithData in validContoursWithData:            # for each contour
        ...
        if contourWithData.intRectWidth >= 12 and (contourWithData.intRectHeight >= 22): 
            ...
            npaROIResized = np.float16(npaROIResized)       # convert from 1d numpy array of ints to 1d numpy array of floats
            numtemp2 = np.concatenate((numtemp2, [str(contourWithData.intRectX) + "=" + str(contourWithData.intRectY) + "=" + str(contourWithData.intRectWidth) + "=" + str(contourWithData.intRectHeight)]))
            #print(str(imgcount -1) + str(npaROIResized))
            if (imgcount <= 2):
                numtemp = np.array(npaROIResized)
            else:
    ...
    cv2.imwrite("testing1.jpg",imgTestingNumbers)
    pred=model.predict(numtemp)
    pred2=[]
    for i in range(pred.shape[0]):
        #print(pred[i])
        pred2.append(list(pred[i]).index(max(pred[i])))
        
    out_file = open("predictions.tsv", "w")
    #out_file.write("ImageId,Label\n")
    for i in range(len(pred2)):
        xtemp = numtemp2[i].split("=")
        out_file.write(str(i+1) + "\t" + str(int(pred2[i])) + "\t" + str(chr(int(pred2[i]))) + "\t"  + str(xtemp[0])  + "\t"  + str(xtemp[1])  + "\t"  + str(xtemp[2])  + "\t"  + str(xtemp[3]) + "\n")
    out_file.close()
